    ...
    <div class="container">
        <div class="row">
            <div class="col-md-12 text-center mt-4">
                ...
                <label for="" class="justify-content-center" style="color: white;font-size:30px">PR -
                    Restaurant</label>
                {{-- <i class="fa-solid fa-2x fa-house text-white me-3 ms-4"></i> --}}
                {{-- <a href="{{ url('order/' . $table) }}">
                    <i class="fa-solid fa-2x fa-basket-shopping text-white me-3 ms-5"></i></a> --}}
            ...
            <div class="col-md-12">
                <ul class="nav nav-pills" id="ex1" role="tablist">
                    ...
                    <li class="nav-item" role="presentation">
                        <a class="nav-link " href="{{ url('order_add/' . $table) }} "><label class="xl"
                                style="color: white">อาหารประเภทต้ม</label> </a>
                    </li>
                    <li class="nav-item" role="presentation">
                        <a class="nav-link " href=" {{ url('order_add_phad/' . $table) }}">&nbsp;&nbsp;<label
                                class="xl" style="color: white">อาหารประเภทผัด</label>&nbsp;&nbsp; </a>
                    </li>
                    <li class="nav-item" role="presentation">
                        <a class="nav-link " href="{{ url('order_add_tod/' . $table) }} ">&nbsp;<label class="xl"
                                style="color: white">อาหารประเภททอด</label>&nbsp;</a>
                    </li>
                    <li class="nav-item" role="presentation">
                        <a class="nav-link active bg-warning" href="{{ url('order_add_drink/' . $table) }}"><label
                                class="xl" style="color: white">เครื่องดื่ม</label> </a>
                    </li>
                    <li class="nav-item" role="presentation">
                        <a class="nav-link" href="{{ url('order_add_nuang/' . $table) }}"><label class="xl"
                                style="color: white">ประเภทนึ่ง</label> </a>
                    </li>
                    <li class="nav-item" role="presentation">
                        <a class="nav-link" href="{{ url('order_add_sod/' . $table) }}"><label class="xl"
                                style="color: white">ประเภทอาหารสด</label> </a>
                    </li>
                    <li class="nav-item" role="presentation">
                        <a class="nav-link" href="{{ url('order_add_pingyang/' . $table) }}"><label class="xl"
                                style="color: white">ประเภทปิ้งย่าง</label> </a>
                    </li>
                </ul>
            </div>
        </div>
                        ...
                        <div class="col-6 col-md-6 col-xl-6">
                            <div class="card-body">
                                <button class="btn btn-outline-danger" href=" " style="height: auto;">
                                    <div class="card-body shadow" style="background-color: rgba(41, 198, 246, 0.301)"
                                        style="width: auto;">
                                        <img src="{{ asset('storage/menu/' . $item->img) }}" height="140px"
                                            width="140px" alt="Image" class="img-thumbnail"
                                            
                                            data-bs-toggle="modal"
                                            data-bs-target=".addModal{{ $item->menukitchen_id }}" 
                                            ><br>
                                        <label for=""
                                            style="font-size:13px;color: rgb(255, 240, 241)">{{ $item->menukitchen_name }}</label><br>
                                        <label for=""
                                            style="font-size:17px;color: rgb(255, 240, 241)">{{ $item->menukitchen_pricesale }}.-</label>
                                    </div>
                                </button>
                            </div>
                        </div>
                        <!-- Modal addModal-->
                        <div class="modal fade addModal{{ $item->menukitchen_id }}" tabindex="-1"
                            aria-labelledby="addModalLabel" aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h1 class="modal-title fs-5" id="addModalLabel">{{ $item->menukitchen_name }}
                                        </h1>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal"
                                            aria-label="Close"></button>
                                    </div>
                                    <form class="custom-validation" action="{{ route('or.order_save') }}"
                                        method="POST" id="order_saveForm" enctype="multipart/form-data">
                                        @csrf

                                        <input type="hidden" name="menukitchen_id" id="menukitchen_id"
                                            class="form-control" value="{{ $item->menukitchen_id }}">
                                        <input type="hidden" name="table" id="table" class="form-control"
                                            value="{{ $table }}">
                                        <input type="hidden" name="price" id="price" class="form-control"
                                            value="{{ $item->menukitchen_pricesale }}">

                                        <div class="modal-body">
                                            <div class="row">
                                                <div class="col-md-12 mb-3">
                                                    <img src="{{ asset('storage/menu/' . $item->img) }}"
